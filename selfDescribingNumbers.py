...
def selfDescribingNumbers(test):
	from collections import Counter

	line = list(map(int, test.strip()))
	c = Counter(line)
	...
	for i, x in enumerate(line):
		if x != c[i]:
			self_d = 0
			break
			# return(0) # not self describing
		# else:
			# return(1) # self describing

	return(self_d)

file = open('selfDescribingNumbers.txt','r')

for test in file:
	print(selfDescribingNumbers(test))
