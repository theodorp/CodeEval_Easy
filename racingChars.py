...
# race track with obstructions, gates and checkpoints. The goal is to find a
# way of passing this track, using the following rules:
...
# line in this file is a new segment of a race track. E.g.
...
# OUTPUT SAMPLE: Print out the way of passing this race track starting from
# the first line of the file. Use a "|" (pipe) for the straight, use a "/"
# (forward slash) for the left turn and use a "\" (back slash) for the right
# turn. E.g.
...
file = open('racingChars.txt','r')

for test in file:
	print(racingChars(test))