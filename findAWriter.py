...
def findAWriter(test):
	line = test.split('|')

	code = line[0].strip()
	key = list(map(int, line[1].split()))

	author = ''.join([code[i-1] for i in key])

	return author 
	# return ''.join([line[0].strip()[i-1] for i in list(map(int, line[1].split()))])

file = open('findAWriter.txt','r')

for test in file:
	print(findAWriter(test))
